...
int main(int argc, char **argv) {
  ...
  cusparseDnVecDescr_t Tempdescriptor; // Descriptor needed by cuSPARSE
  cusparseDnVecDescr_t Tmpdescriptor; // Descriptor needed by cuSPARSE
  ...
  if (concurrentAccessQ) {
    cputimer_start();
    //@@ Insert code to prefetch in Unified Memory async
  ...
  cusparseCheck(cusparseSpMV_bufferSize(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, 
                          &one, Adescriptor, Tempdescriptor, &zero, Tmpdescriptor,
