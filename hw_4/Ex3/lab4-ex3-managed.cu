...
int main(int argc, char **argv)
{
  ...
  cudaMallocManaged(&hostMem, totalSizeA + totalSizeB + totalSizeC);

  hostA = &hostMem[0];
  hostB = &hostMem[numARows * numAColumns];
  hostC = &hostMem[numARows * numAColumns + numBRows * numBColumns];
  ...
  gemm<<<dim3(blockNumX, blockNumY, 1), dim3(threadPerBlockX, threadPerBlockY, 1)>>>(hostA, hostB, hostC, numARows, numAColumns, numBRows, numBColumns);
